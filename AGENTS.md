...
| Area | Status |
|---|---|
| `apps/web` | **Built and polished, running on LIVE backend data.** Next.js 15. Signals, signal detail, confidence, live price, Track Record, Insights (news + risk flags), Notifications and most of the Admin console read the real API over HTTP + Socket.IO; remaining mocks are the surfaces whose backend does not exist yet (Settings and the auth-dependent Admin tabs), each labelled honestly. The Scanner is now LIVE (M15) and the top bar shows the real regime + feed connection (no more "Phase 2" placeholder). Zero lint errors, strict TypeScript, build green. Seven workspaces: Signals · Scanner · Strategies · Insights · Track Record · Notifications · Settings (+ Admin). |
...
| **Live Scan Engine** | **BUILT (M15).** The top of the pipeline that was missing — the continuous loop that makes the platform LIVE. On a configurable interval (`SCAN_INTERVAL_MS`, default 90s) a `ScanWorker` sweeps a bounded, liquidity-ordered universe across **every enabled exchange (Binance + Bybit; OKX stays disabled)**, and a `ScanOrchestrator` runs the SAME engines the confidence replay proved — indicators → patterns → regime → strategy → risk → confidence — on the **newest closed bar** with **live order book + ticker**, handing risk-approved, confidence-scored candidates to the Signal Engine to publish. It reuses; it re-derives nothing. Bounded on purpose (`SCAN_MAX_SYMBOLS`, default 60) so a broad scan never trips a rate-limit ban. Crash-proof: a sweep that throws is logged and retried next tick, never killing the worker. The historical **backfill is now OFF by default** (`SIGNAL_BACKFILL_ON_BOOT=false`) — the live scan is the source of truth, and 2024 corpus entries would read as already-invalidated. The Settlement Worker now also **drops an invalidated setup fast** (stop breached before entry → CANCELLED on the next 30s sweep) so the feed self-clears within the owner's five-minute rule. Exposes `/api/v1/scan` (GET latest sweep, POST on-demand). |
| **Identity & Access (Auth/RBAC)** | **BUILT (M16).** The platform now knows who is asking. `User` + `UserPreferences` in Postgres; registration, login, `/auth/me`, change-password and per-user preferences under `/api/v1/auth/*`. Passwords are **scrypt**-hashed with a per-user salt (Node crypto — no bcrypt/argon native build); sessions are small **HS256** JWTs signed with Node crypto (no `jsonwebtoken` dep, `alg` never trusted). **The first account to register becomes ADMIN** (a fresh install claims its own operator); everyone after is a TRADER. Two guards exported for the platform to compose: `JwtAuthGuard` (who) and `RolesGuard` + `@Roles()` (what). Login never reveals whether an email exists (same error, same timing). Verified against the live API: register→ADMIN+JWT, `/auth/me`, wrong-password→401, no-token→401. Frontend: login/register wired, an `AuthGate` protects `(platform)`, the sidebar shows the real user + sign-out, and Settings → Change Password is live. Refresh tokens, 2FA, admin user-management UI and the RBAC lock-down of existing read endpoints are the follow-ups. |
| **Watchlist (M17)** | **BUILT.** Per-user watchlist stored in `UserPreferences`; `GET/POST/DELETE /api/v1/auth/me/watchlist`. A `WatchlistService.union()` gives the scan the union of every user's watched coins, and the **scan seeds those coins FIRST every sweep, exempt from the symbol cap** — a coin you asked us to watch is never missed for want of budget. It is a priority list, not a filter: you still see every other Prime signal. Frontend: a `WatchlistCard` on the Signals page (add/remove, live). Per-user *push* delivery (Telegram to the specific watcher) lands with Phase 4, where chat-level addressing is natural; today watched-coin signals arrive via the existing in-app feed. |
| **Telegram delivery (M18)** | **BUILT.** Real Bot API sending (dependency-free `TelegramClient` over `fetch`), account **linking with no webhook** — a one-time `/start` code + a long-polling worker binds a chat to a user and saves the chat id to their preferences. `TelegramChannel` resolves user→chat and sends; `NotificationPreferencesProvider` caches each user's channels/quiet-hours/chat (hydrated on boot, refreshed on a `user.preferences.changed` event). **Delivery policy: a user gets a Telegram alert only for a PRIME signal OR a coin on their watchlist** — never a firehose. `/api/v1/telegram/{status,link}` + `DELETE /telegram`; Settings → Integrations → **Telegram connect** is live. Off cleanly until `TELEGRAM_BOT_TOKEN` is set. Guide: [docs/TELEGRAM.md](docs/TELEGRAM.md). WhatsApp/Email remain wired stubs behind the same interface. Verified live: status→configured:false without a token, endpoints guarded, DI graph boots. |
| **Macro & Economic Calendar (M19)** | **BUILT.** The scheduled events that move the whole market at once — FOMC, CPI, NFP. Ships a **built-in FOMC schedule** (works with zero config) and takes a live provider (`ECONOMIC_CALENDAR_API_KEY`, Financial Modeling Prep free tier) for the full calendar with forecasts + actuals. A **macro window** raises when a HIGH-impact release is within 30 min (protect the trader — volatility, stops hit on noise); a deterministic **interpreter** reads the surprise once a number prints (hot CPI → RISK_OFF, a cut → RISK_ON), and it is **context, never a signal** (ADR-023 §5) — it colours the backdrop, the chart still earns the trade. `GET /api/v1/macro/calendar`; a `macro.event.imminent` event → in-app/Telegram warning; Insights page shows the calendar, the Signals page a macro-window banner. Verified live: built-in schedule returns the upcoming FOMC dates, window logic correct. **Social feeds remain an honest placeholder** (real sentiment needs paid X/Twitter access); Reddit-RSS social is a candidate follow-up. |
| **The Honesty Audit + Polish (M20)** | **DONE.** The owner clicked every surface; everything that pretended is now real or gone. **Scanner** no longer sweeps inside a request (a full sweep takes minutes; the first version timed out and looked "unreachable") — `POST /scan` kicks a background sweep and returns the latest result with `inProgress`, the page polls; the sweep itself now runs 4 symbols in parallel. **Admin console**: only real panels remain (Dashboard, Health, **Users — real list/suspend/delete**, Exchanges, Queues, Flags, Audit, Maintenance); the mock Workers/AI-Providers/Monitoring(broken chart)/System-Logs/Roles tabs are **deleted**; Platform Health dropped the fictional AI-Gateway and Object-Storage cards; `AdminGuard` now accepts a signed-in **ADMIN JWT** (operator token stays for machines); the Admin nav hides for non-admins. **Users**: `suspended` on the model — login refuses after the password check, `/auth/me` kills live sessions; admin cannot suspend/delete self. **Settings**: API-Keys (platform never trades for you — irrelevant), Connected-Accounts, privacy toggles, fake About/Account tabs **deleted**; Profile shows the real account; Trading Preferences write real per-user prefs; Appearance is the real theme; Security = the real password change. **Notifications page**: channel toggles write real preferences (the same document the delivery engine reads); dead Configure/Test buttons removed. **Phase 6 polish**: scroll-to-top on every route (owner requirement), per-route fade-up, and a CSS-only dark polish layer (ambient radial washes, card glass-edge + hover lift, Prime-signal glow, primary-button sheen, sidebar accent bar, gradient topbar hairline, on-brand selection). |
| AI layer | **Not built.** |
...
1. ~~**Scanner**~~ — **LIVE (M15).** Wired to `GET/POST /api/v1/scan`; `mock-opportunities.ts` and `mock-scan.ts` are **deleted**. Shows real risk-approved opportunities with live current price, real diagnostics (pairs checked / passed / duration), Binance + Bybit only. Per-scan strategy selection and the rejection ledger are the remaining follow-ups (the scan runs the user's active strategies for now).
2. **Insights** — News + risk flags are LIVE (M12). Social/fundamentals show an honest "not live yet" state (architecture-only).
3. **Settings** — **Change Password is LIVE (M16)** and per-user preferences persist server-side; the rest of the Settings surfaces (2FA, sessions, appearance, integrations) stay honest placeholders. **Admin is now PARTLY LIVE (M14):** dashboard, platform health, exchanges, queues, feature flags (interactive), audit log and maintenance mode read the real `/admin/*` API; users/roles/strategy-admin/workers/providers/monitoring/system-logs stay honest placeholders behind a `NotLiveBanner`. Notifications LIVE (M13).
